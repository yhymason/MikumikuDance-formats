...
std::unique_ptr<PmdModel> pmd::PmdModel::LoadFromStream(std::ifstream * stream)
{
	...
	std::vector<std::string> bone_english;
	std::string eng[19] = {"center", "upper body", "head", "lower body", "waist accessory"
							"shoulder_L", "arm_L", "elbow_L", "wrist_L",
							"leg_L", "knee_L", "ankle_L", "shoulder_R", "arm_R", "elbow_R", "wrist_R",
	...
	std::ofstream myfile;
	myfile.open ("bone_out.txt");
	for(size_t i = 0; i < len; i++){
		std::string str = result->bones[i].name_english;
