...
namespace vmd
{
	...
	public:
		...
		static std::unique_ptr<VmdMotion> LoadFromStream(std::ifstream *stream)
		{
			...
			size_t len = result->bone_frames.size();
			std::ofstream myfile;
  			myfile.open ("motion_out.txt");
			for(size_t i = 0; i < len; i++){
				myfile << "frame:" << result->bone_frames[i].frame << "\n";
				myfile << "bone:" << result->bone_frames[i].name << "\n";
				myfile << "x:" << result->bone_frames[i].position[0] << "\n";
				myfile << "y:" << result->bone_frames[i].position[1] << "\n";
				myfile << "z:" << result->bone_frames[i].position[2] << "\n";
				myfile << "qx:" << result->bone_frames[i].orientation[0] << "\n";
				myfile << "qy:" << result->bone_frames[i].orientation[1] << "\n";
				myfile << "qz:" << result->bone_frames[i].orientation[2] << "\n";
				myfile << "qw:" << result->bone_frames[i].orientation[3] << "\n";
			}
 			myfile.close();
			return result;
		}
